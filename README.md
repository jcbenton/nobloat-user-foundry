...
With the "force change for weak passwords" feature enabled, a user whose password was flagged weak (grace period expired) was blocked at login with *"Please reset your password to continue"* — yet resetting the password never cleared the weak-password flag. The weak-password gate (`NBUF_Hooks::enforce_verification_before_login`, priority 25) runs *before* the validator that clears the flag for a now-compliant password (`NBUF_Password_Validator::check_password_at_login`, priority 29), so once the grace period expired the clearing code never ran and the account stayed locked out no matter how many times the user reset. The reset-link flow (`after_password_reset`), the account-page change (`wp_set_password`, cleared inline), and admin user edits (`profile_update`) now clear both the `weak_password_flagged_at` and `force_password_change` flags on every genuine password change, so a reset reliably restores access. Already-locked accounts are freed by resetting, or by a one-time `UPDATE wp_nbuf_user_data SET weak_password_flagged_at = NULL, force_password_change = 0`.

### 1.7.42 — A passkey login can never bounce you to wp-login.php

An out-of-band login (passkey, magic link, or 2FA completion) resolves the account from a stored credential or token. A stale or malformed record could carry a `user_id` that no longer maps to a real user (for example an old passkey row created under an earlier version). The shared login gate never confirmed the user existed, so the flow set a WordPress auth cookie for an account core could not load — and the next `/wp-admin/` request bounced the user straight to the backend `wp-login.php` login screen, with no explanation. Added an explicit user-existence check at the top of `NBUF_Auth::enforce_login_status()` (the single gate every out-of-band path shares): a zero or orphaned `user_id` is now rejected with a clear inline message instead of minting an unusable session. Deleting and re-creating the affected passkey was the previous workaround; it is no longer needed.
