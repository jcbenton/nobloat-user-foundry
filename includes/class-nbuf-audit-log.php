...
class NBUF_Audit_Log {
	 ...
	 */
	public static function log( $user_id, $event_type, $event_status, $message = '', $metadata = array() ) {
	 ...
	 */
	private static function should_track_event( $event_type ) {
		...
		$tracking_settings = NBUF_Options::get( 'nbuf_audit_log_events', array(
			'authentication' => true,
			'verification'   => true,
			'passwords'      => true,
			'2fa'            => true,
	 ...
	 */
	public static function get_logs( $filters = array(), $limit = 25, $offset = 0, $orderby = 'created_at', $order = 'DESC' ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'nbuf_user_audit_log';
	...
	public static function get_stats() {
		...
		/* Oldest entry */
		$oldest = $wpdb->get_var( "SELECT created_at FROM {$table_name} ORDER BY created_at ASC LIMIT 1" );
