...
class NBUF_Hooks {
    ...
    public static function handle_email_change($user_id, $old_user_data) {
        $user = get_userdata($user_id);
        if (!$user || empty($old_user_data->user_email)) return;
        ...
        if (strcasecmp($user->user_email, $old_user_data->user_email) !== 0) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("[NoBloat EV] Re-verification triggered for user ID {$user_id} (email changed).");
            }
        }
    }
    ...
    /* ==========================================================
       ...
       ========================================================== */
    public static function enforce_verification_before_login($user, $username, $password) {
    ...
    public static function handle_default_wordpress_redirects() {
        ...
        if ( $action === 'lostpassword' && $redirect_lostpassword ) {
        ...
        if ( $action === 'rp' && $redirect_resetpass ) {
        ...
        if ( $action === 'register' && $redirect_register ) {
        ...
        if ( $action === 'logout' && $redirect_logout ) {
            ...
            if ( $page_id ) {
                add_filter( 'logout_redirect', function() use ( $page_id ) {
        ...
        if ( $action === 'login' && $redirect_login ) {
	...
	public static function validate_profile_password( $errors, $update, $user ) {
		...
		$password = wp_unslash( $_POST['pass1'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	 ...
	 */
	public static function validate_registration_password( $errors, $sanitized_user_login, $user_email ) {
		...
		$password = $_POST['pass1']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
