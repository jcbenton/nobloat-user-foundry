...
class NBUF_User_2FA_Data {
	 ...
	 */
	public static function get_backup_codes_used( $user_id ) {
		$data = self::get( $user_id );
	 ...
	 */
	public static function get_last_used( $user_id ) {
		$data = self::get( $user_id );
	 ...
	 */
	public static function get_forced_at( $user_id ) {
		$data = self::get( $user_id );
	 ...
	 */
	public static function is_setup_completed( $user_id ) {
		$data = self::get( $user_id );
		return $data && $data->setup_completed == 1;
	}
	 ...
	 */
	public static function enable( $user_id, $method, $totp_secret = null ) {
		$data = array(
			'enabled'           => 1,
			'method'            => $method,
			'setup_completed'   => 1,
	 ...
	 * @return   bool                   True on success, false on failure
	 */
	public static function set_totp_secret( $user_id, $secret ) {
	 ...
	 * @return   bool                 True on success, false on failure
	 */
	public static function set_backup_codes( $user_id, $codes ) {
		return self::update( $user_id, array(
	 ...
	 * @return   bool                True on success, false on failure
	 */
	public static function mark_backup_code_used( $user_id, $index ) {
	 ...
	 * @return   bool                   True on success, false on failure
	 */
	public static function add_trusted_device( $user_id, $token, $device ) {
	 ...
	 * @return   bool                   True on success, false on failure
	 */
	public static function remove_trusted_device( $user_id, $token ) {
		$devices = self::get_trusted_devices( $user_id );
	 ...
	 * @return   bool                True on success, false on failure
	 */
	public static function set_last_used( $user_id ) {
		return self::update( $user_id, array(
	 ...
	 * @return   bool                      True on success, false on failure
	 */
	public static function set_forced_at( $user_id, $forced_at ) {
		return self::update( $user_id, array(
			'forced_at' => $forced_at,
		) );
	}
	...
	public static function update( $user_id, $data ) {
		...
		if ( isset( $data['enabled'] ) ) {
			$was_enabled = $exists && $exists->enabled == 1;
			$now_enabled = $data['enabled'] == 1;
