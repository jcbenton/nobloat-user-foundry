			...
			try {
				...
				foreach ( $bp_user_data as $row ) {
					...
					if ( isset( $field_mapping[ $row->field_id ] ) ) {
						...
						if ( ! in_array( $row->field_id, $results['fields_unmapped'], true ) ) {
	...
	private static function build_field_mapping( $bp_fields ) {
		...
		foreach ( $bp_fields as $bp_field ) {
			...
			foreach ( $nbuf_flat as $nbuf_key => $nbuf_label ) {
				...
				if ( strpos( $bp_name_normalized, $nbuf_label_normalized ) !== false ) {
					$mapping[ $bp_field->id ] = $nbuf_key;
					continue 2;
				}

				/* Check if NBUF label contains BP name */
				if ( strpos( $nbuf_label_normalized, $bp_name_normalized ) !== false ) {
					$mapping[ $bp_field->id ] = $nbuf_key;
					continue 2;
				}
			}
		...
		foreach ( $users as $user ) {
			...
			$bp_data = $wpdb->get_results(
				$wpdb->prepare(
					...
					FROM {$bp_data_table} d
					INNER JOIN {$bp_fields_table} f ON d.field_id = f.id
					WHERE d.user_id = %d
					AND d.value != ''
					AND f.id != 1",
					$user->user_id
