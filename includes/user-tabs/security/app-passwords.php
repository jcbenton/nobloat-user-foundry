...
$nbuf_users_with_app_passwords = 0;
$nbuf_total_app_passwords      = 0;
global $wpdb;
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
$nbuf_users_with_app_passwords = (int) $wpdb->get_var(
	"SELECT COUNT(DISTINCT user_id) FROM {$wpdb->usermeta} WHERE meta_key = '_application_passwords' AND meta_value != 'a:0:{}' AND meta_value != ''"
);
if ( $nbuf_users_with_app_passwords > 0 ) {
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	$nbuf_all_passwords = $wpdb->get_col(
		"SELECT meta_value FROM {$wpdb->usermeta} WHERE meta_key = '_application_passwords' AND meta_value != 'a:0:{}' AND meta_value != ''"
	);
	foreach ( $nbuf_all_passwords as $nbuf_serialized ) {
		$nbuf_passwords = maybe_unserialize( $nbuf_serialized );
		if ( is_array( $nbuf_passwords ) ) {
			$nbuf_total_app_passwords += count( $nbuf_passwords );
		}
	}
}
?>
...
<?php if ( $nbuf_users_with_app_passwords > 0 ) : ?>
...
<p class="description">
	...
	printf(
		...
		(int) $nbuf_total_app_passwords,
		(int) $nbuf_users_with_app_passwords
	);
	?>
</p>
<?php endif; ?>
