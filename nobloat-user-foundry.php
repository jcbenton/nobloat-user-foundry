...
/* ==========================================================
   PSR-4 AUTOLOADER
   ----------------------------------------------------------
   Automatically loads classes when first used.

   Converts: NBUF_User_Data → class-nbuf-user-data.php

   Performance: Only loads classes that are actually used,
   reducing overhead by 60-80% on most page loads.
   ========================================================== */
 ...
 */
function nbuf_autoload($class) {
	// Only autoload NBUF_ prefixed classes
	if (strpos($class, 'NBUF_') !== 0 && strpos($class, 'Abstract_NBUF_') !== 0) {
		return;
	}
	...
	// Abstract_NBUF_Migration_Plugin → abstract-nbuf-migration-plugin.php
	$class_lower = strtolower($class);
	$class_file = str_replace('_', '-', $class_lower);

	// Add 'class-' prefix only for NBUF_ classes (not Abstract_)
...
/* ==========================================================
   ...
   ========================================================== */
register_activation_hook(__FILE__, ['NBUF_Activator', 'run']);
...
/* ==========================================================
   DISABLE DEFAULT NEW USER EMAILS (only if system enabled)
   ========================================================== */
add_filter('wp_send_new_user_notifications', function($send) {
...
add_action('plugins_loaded', function() {
    ...
    $email_2fa_enabled = NBUF_Options::get('nbuf_2fa_email_method', 'disabled') !== 'disabled';
    $totp_2fa_enabled = NBUF_Options::get('nbuf_2fa_totp_method', 'disabled') !== 'disabled';
...
/* ==========================================================
   LOGIN LIMITING - LAZY INITIALIZATION
   ----------------------------------------------------------
   Only initialize on login pages to avoid unnecessary hooks.
   Autoloader loads the class on-demand.
   ========================================================== */
...
/* ==========================================================
   FRONT-END STYLES
   ----------------------------------------------------------
   Load CSS only on specific plugin pages (if enabled)
   ========================================================== */
...
/* ==========================================================
   ADMIN NOTICE - Missing Templates
   ========================================================== */
add_action('admin_notices', function() {
    ...
    if (empty(trim($html_template)) || empty(trim($text_template))) {
        echo '<div class="notice notice-warning"><p><strong>NoBloat User Foundry:</strong> Default email templates are missing or empty. Please configure them under <em>NoBloat Foundry → User Settings</em>.</p></div>';
    }
});
...
/* ==========================================================
   ...
   ----------------------------------------------------------
   Hide page titles on shortcode pages when using Bricks theme
   ========================================================== */
add_filter('bricks/default_page_title', function ($title) {
    ...
    if (
        has_shortcode($content, 'nbuf_verify_page') ||
